...
function IntakeGroupSelect({ selectedIntakeGroup, setSelectedIntakeGroup }) {
  ...
  const selectedIntakeGroupTitle =
    intakeGroups.find((group) => group.id === selectedIntakeGroup)?.title ||
    "Select an intake group";
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      <div className="flex items-center gap-2 p-2 border border-gray-300 rounded-md bg-white shadow-sm">
        ...
        >
          {isOpen ? (
            <FiChevronUp className="inline-block text-gray-700" />
          ) : (
            <FiChevronDown className="inline-block text-gray-700" />
          )}
        </button>
      </div>
      {isOpen && (
