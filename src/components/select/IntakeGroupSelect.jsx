...
function IntakeGroupSelect({ selectedIntakeGroups, setSelectedIntakeGroups }) {
  ...
  const intakeGroups = intakeGroupsData
    ? Object.values(intakeGroupsData.entities)
    : [];
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <label className="block text-sm font-medium text-gray-700 dark:text-dmgray-200 mb-1">
        Intake Groups
      </label>
      <div className="flex flex-wrap items-center gap-2 p-2 border border-gray-300 dark:border-dmgray-700 rounded-md bg-white dark:bg-dmgray-900 shadow-sm">
        {(selectedIntakeGroups || []).map((groupId) => {
          ...
          return (
            group && (
              <div
                key={groupId}
                className="flex items-center gap-1 bg-gray-100 dark:bg-dmgray-700  px-2 py-1 rounded"
              >
                <span className="text-gray-700 dark:text-dmgray-200">
                  {group.title}
                </span>
                <FiX
                  className="cursor-pointer text-gray-700 dark:text-dmgray-200"
                  onClick={() => removeIntakeGroupSelection(groupId)}
                />
              </div>
            )
          );
        ...
        >
          {isOpen ? (
            <FiChevronUp className="inline-block text-gray-700 dark:text-dmgray-200" />
          ) : (
            <FiChevronDown className="inline-block text-gray-700 dark:text-dmgray-200" />
          )}
        </button>
      </div>
      {isOpen && (
        <div className="absolute z-10 mt-1 w-full border border-gray-300 dark:border-dmgray-700 rounded-md bg-white dark:bg-dmgray-900 shadow-lg">
          ...
          <ul className="max-h-60 overflow-auto">
            {filteredIntakeGroups.map((group) => (
              <li
                key={group.id}
                className="flex items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-dmgray-700 cursor-pointer"
