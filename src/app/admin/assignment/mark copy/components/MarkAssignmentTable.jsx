...
const MarkAssignmentTable = () => {
  ...
  const results = resultsNormalized
    ? Object.values(resultsNormalized.entities)
    : [];
  ...
  const transformedResults = results.map((result) => {
    ...
    const lecturerName = `${
      result.assignment?.lecturer?.profile?.firstName || "N/A"
    } ${result.assignment?.lecturer?.profile?.lastName || ""}`;
    const outcomeTitle =
      result.assignment?.outcome?.length > 0
        ? result.assignment.outcome.map((outcome) => outcome.title).join(", ")
        : "N/A";
    ...
    return {
      ...
      studentName: `${result.student?.profile?.firstName || "Unknown"} ${
        result.student?.profile?.lastName || ""
      }`,
      testName: result.assignment?.title || "Unknown",
      createdBy: lecturerName,
      outcome: outcomeTitle,
      dateWritten: result.dateTaken
        ? new Date(result.dateTaken).toLocaleDateString()
        : "N/A",
    };
  });

  const sortedTransformedResults = transformedResults.sort(
    (a, b) => new Date(b.dateWritten) - new Date(a.dateWritten)
  );
        ...
        <div className="flex space-x-2">
          <button
            onClick={(event) => {
              ...
              window.open(
                `/admin/assignment/mark/${row.original._id}`,
                "_blank"
              );
            }}
